...
#define CREATE_JUMP(name, hookAddress, targetAddress) \
	unsigned char name[sizeof(JUMP_CODE)]; \
	MemoryCopy(name, (unsigned char*)JUMP_CODE, sizeof(JUMP_CODE)); \
	*(unsigned long long*)((unsigned long long)name + 1) = (unsigned long)targetAddress - ((unsigned long)hookAddress + 5)
...
static int EnableHook(HookInformation* information)
{
    ...
    CREATE_JUMP(originalJump, (unsigned char*)buffer + information->BytesToCopy, (unsigned char*)information->OriginalFunction + information->BytesToCopy);
    MemoryCopy(buffer + information->BytesToCopy, originalJump, sizeof(JUMP_CODE));

    CREATE_JUMP(targetJump, information->OriginalFunction, information->TargetFunction);
