...
#define HOOK_C (*b & 0xF)
	...
	struct HookInformation {
		int Enabled = 0;
		int BytesToCopy = 0;
		unsigned char OriginalBuffer[32] = { 0 };
		void* OriginalFunction = nullptr;
		void* TargetFunction = nullptr;
		void* Trampoline = nullptr;
	};

	static const unsigned char JUMP_CODE[] = { 0xE9, 0x00, 0x00, 0x00, 0x00 };
	...
	inline int GetInstructionSize(const void* address) {
		...
		for (int i = 0; i < 14 && FindByte(PREFIXES, sizeof(PREFIXES), *b) || HOOK_R == 4; i++, b++) {
			if (*b == 0x66) operandPrefix = 1;
			else if (*b == 0x67) addressPrefix = 1;
			else if (HOOK_R == 4 && HOOK_C >= 8) rexW = 1;
		}
		...
		if (*b == 0x0F) {
			...
			if (*b == 0x38 || *b == 0x3A) {
				...
				if (HOOK_R == 8) offset += 4;
				else if ((HOOK_R == 7 && HOOK_C < 4) || *b == 0xA4 || *b == 0xC2 || (*b > 0xC3 && *b <= 0xC6) || *b == 0xBA || *b == 0xAC) offset++;
			...
			if ((HOOK_R == 0xE && HOOK_C < 8) || (HOOK_R == 0xB && HOOK_C < 8) || HOOK_R == 7 || (HOOK_R < 4 && (HOOK_C == 4 || HOOK_C == 0xC)) || (*b == 0xF6 && !(*(b + 1) & 48)) || FindByte(OP1_IMM8, sizeof(OP1_IMM8), *b))
				offset++;
			else if (*b == 0xC2 || *b == 0xCA) offset += 2;
			else if (*b == 0xC8) offset += 3;
			else if ((HOOK_R < 4 && (HOOK_C == 5 || HOOK_C == 0xD)) || (HOOK_R == 0xB && HOOK_C >= 8) || (*b == 0xF7 && !(*(b + 1) & 48)) || FindByte(OP1_IMM32, sizeof(OP1_IMM32), *b)) {
				if (*b == 0xB8 || (*b >= 0xB8 && *b <= 0xBF)) offset += rexW ? 8 : 4;
				else if (*b == 0xC7) offset += 4;
				else if (*b == 0x69) offset += 4;
				else offset += (operandPrefix) ? 2 : 4;
			}
			else if (HOOK_R == 0xA && HOOK_C < 4) offset += (rexW) ? 8 : (addressPrefix ? 2 : 4);
			else if (*b == 0xEA || *b == 0x9A) offset += operandPrefix ? 4 : 6;
	...
	inline HookInformation CreateHook(void* originalFunction, void* targetFunction) {
		...
		while (size < sizeof(JUMP_CODE)) {
			size += GetInstructionSize(reinterpret_cast<unsigned char*>(originalFunction) + size);
		}

		information.BytesToCopy = size;
		memcpy(information.OriginalBuffer, originalFunction, size);
		return information;
	}

	inline int EnableHook(HookInformation* information) {
		if (information->Enabled) return 1;

		const int bufferSize = sizeof(JUMP_CODE) + information->BytesToCopy;
		unsigned char* buffer = reinterpret_cast<unsigned char*>(VirtualAlloc(0, bufferSize, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE));
		if (!buffer) return 0;

		information->Trampoline = buffer;
		memcpy(buffer, information->OriginalBuffer, information->BytesToCopy);

		auto createJump = [](unsigned char* codeBuffer, uintptr_t hookAddr, uintptr_t targetAddr) {
			memcpy(codeBuffer, JUMP_CODE, sizeof(JUMP_CODE));
			*reinterpret_cast<unsigned long*>(codeBuffer + 1) = static_cast<unsigned long>(targetAddr - (hookAddr + 5));
			};

		createJump(buffer + information->BytesToCopy, reinterpret_cast<uintptr_t>(buffer) + information->BytesToCopy, reinterpret_cast<uintptr_t>(information->OriginalFunction) + information->BytesToCopy);

		unsigned char targetJump[sizeof(JUMP_CODE)];
		createJump(targetJump, reinterpret_cast<uintptr_t>(information->OriginalFunction), reinterpret_cast<uintptr_t>(information->TargetFunction));

		DWORD originalProtection;
		VirtualProtect(information->OriginalFunction, information->BytesToCopy, PAGE_EXECUTE_READWRITE, &originalProtection);
		memcpy(information->OriginalFunction, targetJump, sizeof(JUMP_CODE));
		VirtualProtect(information->OriginalFunction, information->BytesToCopy, originalProtection, &originalProtection);
